...
export default function Department() {
  ...
  useEffect(() => {
    const managerFunction = async () => {
      const managerRespone = await fetch(
        "http://localhost:4000/api/admin/manager"
      );
    ...
    const employeeFunction = async () => {
      const employeeResponse = await fetch(
        "http://localhost:4000/api/admin/employee"
      );
      ...
      <section className="section-body">
        ...
        >
          ...
          <select name="manager" id="">
            {isManager.map((p, index) => {
              return (
                <>
                  <option value={p._id} ref={useManager} key={index}>
                    {p.Firstname}
                  </option>
                </>
              );
